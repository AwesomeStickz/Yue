...
export const run = async (client: Client, oldMessage: Message, newMessage: Message): Promise<Message | void> => {
    ...
    try {
        ...
        if (commandObj.config.args > args.length) return newMessage.channel.send(`Invalid arguments. Correct usage: \`${prefix}${(commands.get(command) as any).help.usage}\``);
        ...
        await commandFile.run(newMessage, client, args);
